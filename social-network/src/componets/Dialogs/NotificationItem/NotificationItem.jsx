...
const NotificationItem = (props) => {
	return (
		<NavLink exact to={ props.link } className={style.notificationItem} activeClassName={style.linkActive}>
			<div className={style.logo}>
				<img src="https://pbs.twimg.com/profile_images/1167923688450052096/wvr50pbL_400x400.jpg" alt="logo" />
				<div className={style.messageCount}>{ props.messageCount }</div>
			</div>
			<div className="headerBlock">
				<p className={style.name}> { props.userName }</p>
				<div className={style.lastMessage}>{ props.lastMessage }</div>
			</div>
			<svg className={`${style.iconMessage} ${style.iconMessage__active}`} viewBox="0 0 40 32">
